...
# (genmech/robots/), so the scene follows the configured hand.
#
...
def setup_student_camera(env) -> None:
    ...
    if backend in ("tiled", "standard"):
        ...
        # Robot link visuals come from the spec so this follows the hand.
        dynamic_exprs = tuple(cfg.raycast_dynamic_prim_exprs) + tuple(
            env.robot_spec.link_prim_regexes
        )
        for expr in dynamic_exprs:
...
def _run_foundation_stereo(env, cfg) -> torch.Tensor:
    ...
    if getattr(env, "_fs_module", None) is None:
        from genmech.perception.fast_foundation_stereo import (
