...
# Palm body AFTER merge_fixed_joints.
PALM_BODY = "iiwa14_link_7"

# Identical to SHARPA's.
PALM_CENTER_OFFSET = (-0.0, -0.02, 0.16)

# A ghosted joint gets its slot's REAL actuation -- the same stiffness, damping and armature...
...
def hand_joint_names() -> tuple[str, ...]:
    ...
    return tuple(
        ...
        for slot in params.JOINT_SLOTS
    )
...
def template_adjacent_links(hand: params.HandParams | None = None) -> dict[str, list[str]]:
    ...
    * Filter too little and rigidly-adjacent links grind. On SHARPA's thumb the
    ...
    for i in range(params.N_FINGER_SLOTS):
        ...
        # The palm overlaps whatever the finger presents at its base.
...
def synth_spec(hand: params.HandParams, *, ensure_urdf: bool = True) -> RobotSpec:
    ...
    for name in names:
        ...
        slot_on = dict(zip(params.JOINT_SLOTS, fp.enabled))[_slot_of(name)]
        active[name] = fp.active and slot_on

    def table(src: dict[str, float], _unused: float = 0.0) -> dict[str, float]:
        # Same value whether the joint is live or ghosted: a locked joint is still a real mechanism,...
        return {n: src[_slot_of(n)] for n in names}

    # Fingertips: active fingers only.
    tip_bodies: list[str] = []
    ...
    for i, fp in enumerate(hand.fingers):
        # The capsule runs along +x from the link origin, so its distal cap -- the part that actually...
    ...
    return RobotSpec(
        ...
        hand_stiffness=table(robot_spec.SLOT_STIFFNESS),
        hand_damping=table(robot_spec.SLOT_DAMPING),
        hand_armature=table(robot_spec.SLOT_ARMATURE),

        arm_default_joint_pos=ARM_DEFAULT_JOINT_POS,
        # Zero is inside every slot's limits, active or ghosted: SHARPA's own home pose is all-zeros...
        ...
        # Capsules: URDF has no capsule primitive, so build_hand_urdf emits cylinders and the importer...
...
def params_for_name(name: str) -> params.HandParams:
    ...
    # sample_population draws sequentially from one RNG, so element i depends on every draw before...
    rng = random.Random(seed)
    hand = None
    for i in range(index + 1):
        hand = params.sample_valid(rng, name=f"gen_{seed:04d}_{i:03d}")
...
def get_generated_spec(name: str) -> RobotSpec:
    ...
    if hand.name != name:
        # SHARPA_LIKE is authored as "sharpa_like"; the registry name carries the gen_ prefix so the...
