...
def urdf_for_viewing(hand: design_space.Hand, out_path) -> "pathlib.Path":
    ...
    Only the links a design actually has: ghosts would draw as specks at the
    palm. Geometry is the capsule the collider uses, drawn as a cylinder, which
    is what the viewer can render.
    """
    import pathlib
        ...
        for d, seg in enumerate(finger.segments):
            ...
            for tag in ("visual", "collision"):
                ...
                ET.SubElement(node, "origin", {
                    "xyz": f"{seg.length / 2.0} 0 0", "rpy": f"0 {math.pi / 2.0} 0"})
                geom = ET.SubElement(node, "geometry")
                ET.SubElement(geom, "cylinder", {
                    "radius": f"{r}", "length": f"{max(seg.length, 1e-4)}"})
            parent = "palm" if d == 0 else f"f{f}_link{d - 1}"
            local = frames[(f, d)] if d else frames[(f, 0)]
            if d:
                pos = (finger.segments[d - 1].length, 0.0, 0.0)
            ...
            lo, hi = seg.joint.limits or design_space.JOINT_LIMIT
