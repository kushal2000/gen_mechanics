...
def draw_hand(ax, hand: S.Hand, flex_deg: float) -> None:
    ...
    for f in hand.fingers:
        # Drive ACTUATORS, not joints: a rigid partner follows its driver, and
        # passive joints stay at their spring rest rather than being forced.
        inputs = {act[1]: math.radians(flex_deg)
                  for act in S.actuators(f) if act[1][1] == "FE"}
        joints, segs = S.forward_kinematics(f, S.expand_inputs(f, inputs))
        c = FACE_COLOR[f.face]
        for p0, p1, _ in segs:
