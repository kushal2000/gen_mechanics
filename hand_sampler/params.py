...
# Canonical rolls in the regular part of the chain.
...
class FingerParams:
    ...
    # Which of the 6 slots are actuated.
    ...
    # Scales every tier radius on this finger.
    radius_scale: float = 1.0

    # How ``mount`` was generated, when it came from :func:`mount_on_face`: (face, u_frac, v_frac,...
...
# --- mounting on the palm surface ------------------------------------------ Human fingers...
...
# Abduction is the interesting knob.
AA_HALF_RANGE = (D(2.0), D(25.0))

N_FINGERS_CHOICES = (2, 3, 4, 5)

# Order in which a finger's joints come alive as its complexity rises.
...
# A metacarpal is only meaningful once a CMC joint can move it.
...
def mount_on_face(face: str, u_frac: float, v_frac: float, roll: float,
    ...
    # Side faces run down to the flange, so lift any mount that lands in the wrist keep-out.
    ...
    # Any vector not parallel to x_axis works as a reference; the roll below sweeps the remaining...
...
def sample(rng: random.Random, *, name: str, n_fingers: int | None = None,
    ...
    for i, slot in enumerate(slot_names):
        ...
        if i >= n_fingers:
            # Ghosted finger: keep the reference geometry so the URDF stays well-formed, and let...
        ...
        # Only give the finger a metacarpal if a joint can actually move it; otherwise the length...
        radius_scale = _u(rng, RADIUS_SCALE_RANGE)
        # A metacarpal of 0 is a real choice (no metacarpal at all), so keep that branch; only a...
        ...
        limits["MCP_AA"] = (-aa, aa)
        # A slot that SHARPA ghosts carries SHARPA's ghost limits, which would lock the joint even...
        ...
        fingers.append(replace(
            ...
            mount_params=mp,
            # The face mount is the ONLY source of base placement and orientation.
            cmc=IDENTITY,
...
def validate(hand: HandParams) -> None:
    ...
    # Mounts must not coincide, or the finger bases interpenetrate and PhysX spends every step...
